...
def infer_category_from_path(path: Path) -> str | None:
    return infer_category_from_name(path.name) or infer_category_from_name(path.parent.name)
    ...
    for source_root in config.structured_sources:
        ...
        for label_dir in sorted([path for path in source_root.iterdir() if path.is_dir() and path.name in config.labels]):
            for image_path in iter_image_files(label_dir, config.image_extensions):
                category = infer_category_from_path(image_path)
                if category is None:
                    continue
...
def write_dataset(assignments: list[tuple[str, ImageRecord]], config: DataConfig) -> list[SplitAssignment]:
    ...
    try:
        ...
        for split_name, record in assignments:
            ...
            destination = temp_dir / split_name / record.label / record.category / file_name
            destination.parent.mkdir(parents=True, exist_ok=True)
            final_destination = output_dir / split_name / record.label / record.category / file_name
